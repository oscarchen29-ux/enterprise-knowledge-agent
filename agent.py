...
# 年份數字必須跟著出現。原本單獨一個「入學」也算數,結果「學士班新生入學有什麼獎勵?」
# 被當成已經講了屆別 —— 這句話裡根本沒有年份。
...
# 會因入學屆別而不同的主題 = docs/ 裡依屆別分檔或分版本的規定。原本只列共同課程與
# 通識,其餘一律駁回屆別追問,還回饋模型「只有共同課程與通識會因屆別不同」——
# 這句話不成立:碩士班優秀學生獎勵辦法分 112 含以前 / 113 起,學士班新生入學獎勵
# 辦法分 114 含以前 / 115 起,科目表、課程地圖、輔系、雙主修也都按屆別分檔
# (總學分相同,但必修哪些課不一定相同)。問到這些主題時,屆別是必要條件。
_COHORT_SENSITIVE = re.compile(r"共同課程|通識|必修|選修|科目表|課程地圖|輔系|雙主修|獎勵|獎學金|優秀學生")
# 模型問的是哪一類條件
...
def redundant_clarification(question: str, task: str) -> str | None:
    ...
    if _ASKS_COHORT.search(question):
        ...
        if not _COHORT_SENSITIVE.search(task):
            # 這段文字會被模型當成事實讀進去,所以只陳述能確定的事、不下全稱斷言,
            # 並留一條退路:查到的文件若本身依屆別分列,就分別說明,不要只挑一版。
            return ("這一題的主題不屬於依入學學年度分版本的規定(科目表、課程地圖、"
                    "共同與通識學分、獎勵辦法),先不必追問屆別,請直接查詢。若查到的"
                    "文件本身依入學學年度分列不同標準,請在回答中分別說明,不要只挑一個版本。")
    return None
