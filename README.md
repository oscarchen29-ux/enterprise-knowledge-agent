...
大三專題。本地部署的校系行政問答 Agent,以國立暨南國際大學資訊工程學系為場景。

「Agent」指的是:查詢關鍵字由模型自己決定、模型自己在「查文件」與「反問使用者」
兩個工具之間選擇、答完再自我查核。**不是指多步驟自主規劃** —— 實測 96 次執行中
92% 只呼叫一次檢索工具(平均 1.09 次),沒有任何一次跑到步數上限。
...
python web/app.py --host 0.0.0.0   # 開放區網或內網穿透存取
```

對話式介面(訊息串 + 底部輸入框),可以接續提問 —— 問完「大三必修有哪些」再問
「那大一呢」會被正確理解。脈絡只帶最近兩則**提問**,不帶答案(帶著上一個答案會讓
模型整段複述,而且答案本身可能是錯的)。

刻意做了三件事,都不是為了好看:

- **每則回答都標出依據哪幾份文件**,點開可以核對原文片段。這是使用者唯一能識破
  「引用了真文件、答案卻是錯的」那種錯誤的方法。
- **警告條釘在標題下方不會捲走**,明講這不是官方系統且會答錯。
- **每則回答各自有回報按鈕**,聊了幾輪之後還能回頭回報前面的答案。

檢索不到任何文件時,該則回答會直接標紅字「請不要採信」,而且不收合。

### 台灣口語與錯字

`web/normalize.py` 在送進模型前整理提問。動機是實測到的問題:「阿大一呢」的
「阿」被模型當成名字的一部分,連問三次都在反問「請問您要問的是阿大一的必修課程嗎?」。
這不是檢索或推理的問題,是**輸入端的在地化問題** —— 輸出端的簡體字已經有 OpenCC 處理,
輸入端的台灣口語一樣需要處理。

處理句首發語詞(阿、欸、齁、那個)、口語詞(蝦米、醬、咧)、注音簡寫(ㄉ、ㄅ、ㄇ)、
以及校務領域的常見錯字(學份、必休、專提)。三條原則:只改幾乎沒有歧義的
(「在/再」這種一律不碰);只在詞界安全的位置改(「阿拉伯數字」「醬油」不會被改壞);
剩下的交給向量檢索,它對錯字的容忍度比 BM25 高很多。

```bash
python web/normalize.py     # 看實際效果
```

### 附上檔案提問

介面可以附上 PDF 或純文字(成績單、課表等),用來問「我這樣還差多少學分」這種
光靠校方規定答不出來的問題。檔案只存在伺服器記憶體、不寫磁碟(多半是個人資料),
上限 5MB、抽取前 8000 字。

**這類回答風險更高。** 實測給它一份成績單問「還差多少學分」,它先說沒看到已修學分、
又拿大一必修學分去算,得出錯誤的差額。純規定查詢答錯還能靠出處核對,個人資料的
計算沒有出處可對,所以介面會對用到檔案的回答另外標示警告。

### 看使用者回報

回報寫進 `web/feedback.jsonl`,**只存在跑網頁的那台機器上**,不上傳、不進版控
(裡面是學生真實提問,屬個人資料)。兩種看法:

```bash
python scripts/show_feedback.py            # 摘要 + 所有被標為有誤的
python scripts/show_feedback.py --all      # 全部
python scripts/show_feedback.py --export questions.txt   # 只匯出提問,供設計題目用
```

或用瀏覽器看,但**必須自己開啟**(預設關閉,沒設 token 時該路徑一律回 404):

```bash
python web/app.py --admin-token 你自己設一組密碼
# 然後開 http://localhost:5000/admin/feedback?token=你自己設一組密碼
```

預設關閉是刻意的 —— 網頁會透過內網穿透開給全系,不能讓任何人打開網址就看到
學生的提問紀錄。
