...
介面刻意做了三件事,都不是為了好看:**每個答案強制顯示依據的文件片段**(這是唯一能讓
使用者識破「有出處但實質答錯」的方法)、**開頭明講這不是官方系統且會答錯**、
**每個答案下方都有回報按鈕**。回報寫入 `web/feedback.jsonl`(已列入 .gitignore,
因為含學生真實提問)—— 那是這個專題最缺的評估資料。

檢索不到任何文件時,介面會直接標示「這個回答沒有查到任何文件,請不要採信」。
